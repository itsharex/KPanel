...
<template>
  <ModalDialog
    ...
    :title="`删除 ${site?.primaryDomain || ''}`"
    description="确认后将严格调用 kejilion.sh 的 k web del 删除该域名站点。"
    size="small"
    @close="close"
  >
    <form id="site-delete-form" class="form-stack" @submit.prevent="emit('confirm')">
      ...
      <div class="inline-alert inline-alert--danger">
        ...
        <span>将删除该域名的网站目录、Nginx 配置、证书和同名数据库（若存在），此操作无法从面板撤销。</span>
      </div>
    </form>
    <template #footer>
