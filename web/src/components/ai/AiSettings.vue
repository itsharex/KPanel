...
async function saveProvider(){if(form.endpointScope==='private'&&!form.privateConfirmed){error.value='内网/本地 API 需要确认你信任该地址。';return};busy.value='save';error.value='';notice.value='';let created:AIProvider|undefined;try{const body={name:form.name,protocol:form.protocol,...(form.protocol==='openai_compatible'?{apiMode:form.apiMode}:{}),baseUrl:form.baseUrl,endpointScope:form.endpointScope,privateConfirmed:form.privateConfirmed,enabled:form.enabled,...(form.apiKey?{apiKey:form.apiKey}:{})};if(editing.value){const current=props.providers.find(item=>item.id===editing.value);if(!current)throw new Error('API 信息正在刷新，请稍后重试。');await aiApi.providers.update(editing.value,{...body,expectedVersion:current.version});form.apiKey='';notice.value='连接信息已保存。';emit('refresh');return}created=await aiApi.providers.create(body);editing.value=created.id;modelForm.providerId=created.id;form.apiKey='';emit('refresh');busy.value='verify';await aiApi.providers.test(created.id);const synced=await aiApi.providers.sync(created.id);notice.value=`连接成功，已同步 ${synced.length} 个模型。`;emit('refresh')}catch(reason){const message=reason instanceof Error?reason.message:'保存失败';if(created){error.value=`API 已安全保存，但自动验证未完成：${message}`;notice.value='可以检查地址或密钥后重新测试。';emit('refresh')}else error.value=message}finally{busy.value=''}}
async function action(id:string,type:'test'|'sync'|'delete'){busy.value=type+id;error.value='';notice.value='';try{if(type==='test'){await aiApi.providers.test(id);notice.value='连接测试成功。'}else if(type==='sync'){const synced=await aiApi.providers.sync(id);notice.value=`模型同步完成，共 ${synced.length} 个。`}else if(confirm('删除后历史对话仍保留，但相关会话必须切换模型。确认删除？')){await aiApi.providers.remove(id);if(editing.value===id)resetProvider();notice.value='API 已删除。'}emit('refresh')}catch(reason){error.value=reason instanceof Error?reason.message:'操作失败'}finally{busy.value=''}}
async function addModel(){if(!modelForm.providerId||!modelForm.modelId)return;busy.value='model';try{await aiApi.providers.addModel(modelForm.providerId,{modelId:modelForm.modelId,displayName:modelForm.displayName||modelForm.modelId,contextWindow:modelForm.contextWindow,toolCalling:modelForm.toolCalling,enabled:true});modelForm.modelId='';modelForm.displayName='';emit('refresh')}catch(reason){error.value=reason instanceof Error?reason.message:'模型保存失败'}finally{busy.value=''}}
async function toggleModel(item:AIModel){busy.value='model-'+item.id;try{await aiApi.providers.addModel(item.providerId,{modelId:item.modelId,displayName:item.displayName,contextWindow:item.contextWindow,toolCalling:item.toolCalling,enabled:!item.enabled,isDefault:item.isDefault});emit('refresh')}catch(reason){error.value=reason instanceof Error?reason.message:'模型状态保存失败'}finally{busy.value=''}}
...
async function removeMemory(item:AIMemory){if(confirm(`退休记忆“${item.title}”？`)){await aiApi.evolution.removeMemory(item.id);await loadEvolution()}}
...
async function rollbackProcedure(item:AIProcedure){const value=Number(prompt(`回滚“${item.title}”到哪个版本？`,`1`));if(value>0){await aiApi.evolution.updateProcedure(item.id,{rollbackToVersion:value});await loadEvolution()}}
async function removeProcedure(item:AIProcedure){if(confirm(`退休流程“${item.title}”？`)){await aiApi.evolution.removeProcedure(item.id);await loadEvolution()}}
