  ...
  >
    <div class="system-resource-dialog">
      <div v-if="!readable" class="inline-alert inline-alert--warning">
        {{ unavailableReason || '当前 Agent 的 Hosts 适配器未就绪。' }}
      </div>
      ...
      <template v-else-if="snapshot">
        <header class="system-resource-dialog__summary">
          <span>共 {{ snapshot.total }} 条记录</span>
          <span v-if="snapshot.truncated" class="text-warning">列表已截断</span>
          <button class="icon-button" type="button" :disabled="refreshing || running" title="刷新 Hosts" aria-label="刷新 Hosts" @click="load(true)">
            <RefreshCw :size="16" :class="{ spin: refreshing }" />
          </button>
        </header>

        <div v-if="!writable" class="inline-alert inline-alert--warning">
          当前 Agent 仅支持查看 Hosts，写入适配器未就绪。
        </div>

        <form class="system-resource-form system-resource-form--hosts" @submit.prevent="addEntry">
          <label class="field">
            <span>IP 地址</span>
            <input v-model.trim="address" autocomplete="off" placeholder="例如 192.0.2.10" />
          </label>
          <label class="field">
            <span>主机名</span>
            <input v-model="hostnames" autocomplete="off" placeholder="多个主机名用空格分隔" />
          </label>
          <label class="field">
            <span>备注（可选）</span>
            <input v-model="comment" autocomplete="off" maxlength="200" placeholder="用途说明" />
          </label>
        ...
        <EmptyState v-if="!snapshot.entries.length" title="暂无 Hosts 记录" description="可在上方添加本机域名映射。" />
        <div v-else class="system-resource-list">
          <article v-for="entry in snapshot.entries" :key="`${entry.line}-${entry.raw}`" class="system-resource-item">
            ...
            <span class="system-resource-item__meta">第 {{ entry.line }} 行</span>
            <button class="icon-button icon-button--danger" type="button" :disabled="!writable || running" title="删除 Hosts 记录" aria-label="删除 Hosts 记录" @click="removeEntry(entry)">
              <Trash2 :size="16" />
            </button>
          </article>
        </div>
