    ...
    <section class="ai-chat-panel">
      ...
      <template v-else-if="active">
        ...
        <div ref="messagesPane" class="ai-messages" @scroll.passive="onMessagesScroll">
          ...
          <div v-if="!messages.length&&!streamText" class="ai-welcome"><div class="ai-orb ai-orb--small"><Sparkles :size="23"/></div><h2>今天想管理什么？</h2><p>我会先读取真实状态；常规读写自动执行，删除、系统核心、exec 和交互输入仍需确认。</p><div><button @click="input='检查服务器资源与异常状态'">检查服务器状态</button><button @click="input='列出当前运行中的容器并总结健康情况'">查看容器健康</button><button @click="input='检查网站与应用状态，不要执行修改'">巡检网站和应用</button></div></div>
