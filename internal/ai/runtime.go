...
func (r *NativeRuntime) systemPrompt(ctx context.Context, userID string) string {
	...
	prompt += "\nDocker mutation workflow: follow the host_docker_task action-specific Schema branch. Before changing or removing an existing container, image, network, or volume, call its matching read tool and pass the exact current id/name and resourceVersion. Before backup restore/migration read host_docker_backups; before daemon mirror/IPv6 changes read host_docker_environment. image_prune removes only dangling images and never substitutes for image_remove."
