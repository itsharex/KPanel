...
- 概览分别展示 `/var/log` 目录总占用和 journal 占用，并明确 journal 通常已包含在前者中，不能相加。
  `/var/log` 扫描、journal 用量和服务枚举都有独立超时；任一来源不可用时展示真实降级状态，不把空白
  或零值冒充成功。
- 查看范围固定为系统、指定服务、安全事件和最近登录。服务只能从 Agent 返回的真实 `.service` unit
  中精确选择；系统与服务日志提供全部、警告及以上、错误及以上三档级别。每次只允许读取 50、100 或
  200 条，命令输出、单条消息和 API 响应均有固定上限，日志正文不进入审计。
