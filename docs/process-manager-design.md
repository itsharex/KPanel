...
- 状态：实现完成；Go/Web/Linux 构建已验收，部署契约检查受既有 WSL `sha256sum -s` 兼容问题阻塞
- 路由：`/processes`
- 入口：概览实时监控标题栏，与“查看历史”并列，不增加左侧主导航
...
- `scripts/verify-deploy.sh`：WSL Ubuntu 的 GNU `sha256sum` 不支持测试替身转译出的 `-s`，在未进入
  本次功能路径前失败；Windows Git Bash 则被安装器按设计拒绝为非 Linux。该既有验证器兼容问题未在本次范围内修改。
