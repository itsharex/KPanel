...
报告故障的 `kpanel.kejilion.eu.org` 主机不接受当前工作环境保存的 SSH 密钥，因此未读取
其内部配置或凭据。v0.42.1 公开后从公网完成黑盒复核：

- `/`：`200`
- `/ai`：`200`
- `/api/v1/health`：`200`，`version=0.42.1`、`status=ok`
- 未登录 `/api/v1/ai/providers`：`401`，不再返回 `421`

这确认故障机已加载 v0.42.1 并恢复域名入口；登录后的会话、Provider 密钥和真实模型调用
仍需管理员在浏览器登录后检查，公网验收不接触账户或密钥。
