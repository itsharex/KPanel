...
最近一次项目性质、业务规模和发布节奏证据见
[`product-quality-review-2026-08-11.md`](product-quality-review-2026-08-11.md)。该审计是制定本版规则的
事实依据，不替代后续持续采集的数据。
...
公开产物 E2E 证明用户实际下载的镜像；生产观察证明本次部署现场。危险、付费、耗流量或破坏性动作
可在隔离真机完成，生产未执行时如实记录，不要求为形式完整在生产制造风险。
...
- 不设置主观的最短发布间隔。至少按滚动 14 天和最近 20 个版本统计部署频率、提交到生产用时、
  变更失败/回滚/紧急热修复、恢复时间和重复发布原因，结合吞吐与稳定性判断节奏；
...
- [ ] 自动测试、隔离真机、公开产物和生产观察的证据层级没有混写，未验证项已列出。
