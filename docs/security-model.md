...
- 公共 API 不接受宿主机 Shell、绝对宿主机目标路径、原始 Docker Exec 结构或
  Nginx 配置。容器控制台专用端点只接受资源版本和单行命令，并在 Agent 内固定转换为
  无 TTY、限时限量的 `/bin/sh -lc` Exec；命令不进入审计变化或后台任务文件。
...
- Docker 资源默认只读；只有归属证据充分的 Kejilion 资源允许生命周期操作。镜像、网络、卷和清理使用固定
  Docker API，逐项执行资源版本与 KPanel 保护检查；镜像源和 IPv6 更新保留未知 `daemon.json` 键并在重启失败时回滚。
...
- 宿主机终端、外部或危险容器 Exec、交互式 TTY、强制批量删除、
  任意 Compose/daemon.json 文本编辑或系统重装。归属明确且通过危险配置检查的
  容器只开放单次、无 TTY、20 秒和 64 KiB 上限的受审计控制台。
- 任意指定包安装/卸载、白名单外发行版升级、清空日志与临时目录，以及自定义更新/清理命令。
- SSH 旧端口删除、任意 DNS/软件源 URL、任意 sysctl 参数，以及应用安装协议之外的任意脚本执行。
- 未确认域名、归属或 `k web` 布局的网站/数据库/证书/目录删除，以及批量强制删除。
