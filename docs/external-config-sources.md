...
| --- | --- | --- | --- | --- |
| `website-nginx` | 静态站、PHP、IP/端口反代、域名反代、负载均衡、跳转；`internal/sites/managed_template.go` | `k web`；`html.conf`、`reverse-proxy-backend.conf`、域名反代及负载均衡模板 | KPanel `renderManagedConfig()` 自行拼接 | **不合规/冻结**：禁止继续修改或发布新增能力；先改为脚本同源模板/入口 |
| `wordpress-flow` | WordPress；`internal/sites/wordpress.go`、`internal/dockerx/wordpress.go` | `k web` WordPress 流程、LDNMP、证书和站点模板 | 源码包沿用脚本 URL 并固定摘要，但数据库、证书和最终 Nginx 流程由 KPanel 独立实现 | **不合规/冻结**：不能宣称完整同流程；迁移后做双端实机验收 |
...
<!-- external-config-debt:wordpress-flow:blocked -->
