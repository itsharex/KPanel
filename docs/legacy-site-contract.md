...
> 已由 [`PROJECT_RULES.md`](../PROJECT_RULES.md) 废止。当前实现以真实产物互通为准。
...
因此 Panel 不直接调用这些网站函数，也不照搬其下载、证书申请、文本替换和
直接 reload 流程。删除只复用 `web_del()` 的产物范围，并增加归属/布局校验、
资源版本、逐字域名确认、原子暂存、`nginx -t` 和失败回滚。
