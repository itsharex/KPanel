...
原生结果统一写入 `summary`：每个维度只保存实际探针返回的指标，页面可直接回显 CPU、内存、磁盘读写、上下行、平均延迟、抖动、丢包、出口 IP 和 IPv4/IPv6 等结果。首屏按“综合评分 → 性能 → 网络”呈现；其中 `KPanel 体检分 · v1` 是基于本次结果的版本化展示指数（性能 45%、网络 55%），用于快速对比本机状态，不替代硬件横向基准。IP 质量分优先使用 IPING `risk_score` 的反向分数（100 - risk_score），接口没有返回有效风险分时回退为基础信息完整度；原始指标始终同时展示。风险等级是 KPanel 展示映射：`<30` 为低风险、`30-79` 为中风险、`>=80` 为高风险，IPING 官方接口文档未定义这些阈值。
