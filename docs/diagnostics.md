...
原生结果统一写入 `summary`：每个维度只保存实际探针返回的指标，页面可直接回显 CPU、内存、磁盘读写、上下行、平均延迟、抖动、丢包、出口 IP 和 IPv4/IPv6 等结果。首屏按“综合评分 → 性能 → 网络”呈现；其中 `KPanel 体检分 · v1` 是基于本次结果的版本化展示指数（性能 45%、网络 55%），用于快速对比本机状态，不替代硬件横向基准。IP 质量分只表示基础信息完整度，不代表第三方信誉或解锁结论；原始指标始终同时展示。
...
- **已实现**：原生目录、原生任务分支、CPU/内存/磁盘/路由/延迟/测速/IP 基础探针、统一 `summary`、原生优先综合入口、首屏综合评分与“性能/网络”分组、第三方脚本可选降级、前端区分原生与脚本。
