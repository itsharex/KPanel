...
- 复核日期：`2026-08-21`
- 基线提交：`44211d11c40c8617e333f3ff65f951ef15d745ea`
- 基线版本：`v0.90.1`
...
- 发布与治理已采用风险感知 `verify-change`、固定 Linux Runner、后台浏览器作业、依赖新鲜度检测、
  环境用途门禁及独立流程异常指标；`prod-108`/`108` 禁用全部 KPanel 操作，隔离验证和唯一正式部署
  默认使用 `arena-154`。

这些变化扩大了桌面、文件、集群、国际化、应用市场、系统调优、Docker、诊断和 AI 业务域，但没有
改变 KPanel 的产品性质、业务真源、Panel/Agent 权限边界、低配主机定位或“不以安全名义削减管理员
能力”的核心思想。
