...
kejilion-agent（宿主机 systemd 服务）
   ...
   ├─ Docker Engine API
   ├─ /home/web 实际产物
   └─ jobs / audit / resource observations
```
...
Agent 不调用 `/usr/local/bin/k`，不载入 Shell 函数，也不改写脚本。脚本外部变更通过 Docker Events、文件事件与周期对账被发现。
...
- SQLite 只保存用户、Session、任务、审计、资源身份和最近观测。
