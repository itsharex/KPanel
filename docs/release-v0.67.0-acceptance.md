...
- 本版本明确排除内置浏览器后续白屏、事务传播及浏览器能力改造；相关浏览器路径相对 `v0.66.0` 为零差异，浏览器能力保持 `v0.66.0` 原状。

## 候选冻结与门禁

- 候选由 `v0.66.0` 主线 `b96f38d` 创建，仅纳入 `d443e62`、`2b224a5` 和版本提交 `6aaabea`；未从浏览器热修复工作树或其他脏工作树拼装内容。
...
- 升级前 Panel 为 `0.66.0`、healthy、0 restart、OOM false；Relay healthy；Agent 为 `0.66.0 v1alpha1`、active，systemd 无待 reload。
...
- 上线后本机与公网 Panel 均为 `0.67.0/ok/v1alpha1`，本机与公网 Relay 均为 `ok/kpanel-browser-core/v1`；Panel/Relay 均 healthy、0 restart、OOM false；Agent 为 `0.67.0 v1alpha1`、active。
- 生产镜像 revision、version、公开 OCI index、固定脚本提交/摘要、Agent 二进制、Relay 二进制、应用市场配置、权限与资源边界均核对通过。Agent SHA-256 `66938f8f7e6faccd0e1737cc00083108ee3da4d397fcc076e30cb0f890c20b22`；Relay SHA-256 `68570b0e552abba0acfc8dcf35653dcf8385ea8058cd779a0d8f93bfa358ab71`。
...
- `.env`、Compose、Agent 配置、systemd、Agent token、Browser Relay 密钥、AI secret 和集群身份均与升级前备份一致。SQLite quick check、5 个 JSON 和 15342 条 JSONL 解析通过；最近 Panel/Relay/Agent 日志无 panic、fatal、segmentation fault 或 OOM。生产验证日志 SHA-256 `0454c0822a77dd7f30ac07a7f73e2d8f2a6f1628007156c8f9e98acc7a4c721d`。
- 60 次、2 秒间隔持续采样从 `2026-08-12T06:10:55Z` 至 `06:13:11Z` 全部通过，同时覆盖本机/公网 Panel、Relay、容器健康/重启/OOM 和 Agent；采样 SHA-256 `7660061ae81340e563b78615d7c26e64ba080bbed401d058a6cc63598c464b9a`。
...
- 停止 Panel、Relay 与 Agent，按已验证备份恢复 `/home/docker/kpanel`、`/root/apps/kpanel.conf` 及 systemd 链接，执行 `systemctl daemon-reload`，将 `latest` 固定回上一镜像摘要后恢复 Agent、Panel 与 Relay。
- 回滚后复核 `0.66.0` 版本、Panel/Relay 健康、容器重启/OOM、Agent、数据完整性和公网入口。历史 `v0.67.0` Tag、Release 与版本镜像保持不可变。

## 限制与沉淀

- 本次生产没有执行 Docker 新建/删除、Compose 写部署或手机端终端写操作；功能写路径由 154 隔离 L2 和真实 Chrome 候选验收覆盖。
- 内置浏览器的新验收标准尚未满足，因此相关热修复继续保持未上线；本版本只保留 `v0.66.0` 已上线的浏览器实现和能力边界。
