...
- 新建会话必须先选择 API 与模型，不再静默使用第一个可用模型；模型按 Provider 分组展示，会话名称可选。
- 会话内的模型选择器实时包含所有已启用 Provider 的已启用模型。运行中允许切换，但当前 Run 使用启动时快照，新选择明确标记为“下一轮”并只影响后续 Run。
...
- 只读工具自动执行。所有写入、删除、容器 exec 和任务输入都进入 `pending_approval`，批准后才调用 Agent。
...
候选先脱敏、Schema 校验和工具 dry-run，再以 `pending` 保存。只有管理员在“进化提案”批准后才进入系统提示词。流程修订保存新版本；可停用、退休或回滚历史版本。任何已批准流程仍不能绕过写操作逐次确认。
