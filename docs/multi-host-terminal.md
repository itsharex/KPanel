...
- 终端下方提供预输入框，按 Enter 后整行发送；xterm.js 原生键盘输入仍可使用，并以 12 ms
  短批次合并，降低高延迟网络中的逐键请求开销；
...
`/bin/bash -l`，不存在时回退 `/bin/sh -l`，并固定 `TERM=xterm-256color`。
...
- UTF-8 输入、整行预输入、控制键、窗口缩放、输出截断、断线重连和 URL 安全跳转；
