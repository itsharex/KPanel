...
- 状态：基础能力已随 `v0.30.0` 发布，交互与资源边界增强进入 `v0.31.0` 发布验收
- 基线：`v0.29.0` / `2abef3d`
...
| 权限与可写范围 | `os.Root` 固定根 `/home`；保护 KPanel 目录、其祖先目录和内部回收站 |
| systemd 沙箱 | 保留 `ProtectSystem=strict`，显式开放 `/home`；不使用会在部分 systemd 版本中覆盖 `ReadWritePaths=/home` 的 `ProtectHome=read-only` |
| 最坏输入/输出 | JSON 64 KiB；文本编辑 2 MiB；单次上传 512 MiB；列表单页 500 项、最多扫描 20,000 项；批量 100 项 |
| 最大并发 | 上传 2、下载 4，超限立即返回 `429`；复制、移动及回收站操作串行执行；复制预算按整批累计 10,000 项/10 GiB |
| 超时、取消与重试 | 普通 API 保留短超时；文件流使用 45 秒无数据空闲超时和 2 小时硬上限；请求取消后停止传输，上传临时文件失败即清理；写操作不自动重试 |
...
- 单元测试：路径穿越、保护目录及其祖先、符号链接、慢速传输、上传/下载并发、目录分页与
  服务端搜索、超限上传/编辑、资源冲突、目标不覆盖和整批累计复制预算。
