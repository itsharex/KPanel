...
- 状态：基础能力已随 `v0.30.0` 发布，根目录边界已随 `v0.34.0` 发布，回收站生命周期随 `v0.34.4` 发布；压缩/解压已随 `v0.34.7` 发布；远程下载已在 `v0.91.0` 后进入候选实现
- 基线：`v0.29.0` / `2abef3d`
...
临时补位。远程下载首个版本不支持 BT/magnet、批量 URL、自定义 Header/Cookie/代理、私网来源、
自动解压、断点续传、自动重试或跨页面/重启恢复。
...
特殊文件拒绝、累计解压预算，以及取消时清理未提交暂存文件并对提交窗口做结果确认。首版不引入常驻归档任务服务；同步请求可取消且在浏览器
...
- API 使用请求作用域 NDJSON：`connecting -> transferring -> confirming -> complete|error`。已知总量
  显示真实进度，未知总量只显示已接收字节，不伪造百分比。`confirming` 表示 Agent 已结束原子上传、
  Panel 正在核对返回结果，不把它误写成“正在提交”。页面停止、关闭、刷新或断连会请求取消并停止读取；
  尚未提交的隐藏暂存文件会清理，若取消恰逢原子提交窗口则提示刷新真实目录确认。目录不会出现半文件。
- 当前没有可复用的通用持久任务框架，首版不为单一动作新增 Job registry、worker、reconcile、轮询、
  保留期和恢复协议。若后续证据表明多 GiB、超慢链路或跨重启继续是核心需求，再作为独立 L2 任务设计。
...
`name` 可省略，接口不接受 `overwrite`。成功响应为流式 `application/x-ndjson`；URL 不进入 path、
query、Agent 请求或通用 Jobs 列表。预流式校验失败使用 RFC 7807，连接后的业务失败使用稳定事件
`code`，前端不得显示底层含 URL 的错误字符串。
...
- 顶部命令栏直接显示“远程下载”。弹窗只包含下载地址、可选保存名称和当前目录快照；提交后立即
  清空 URL 输入并关闭弹窗，文件区顶部显示安全名称、目标目录、阶段、真实字节进度和停止按钮。
  界面明确说明保持当前文件页面打开，关闭或刷新会请求停止；停止与原子提交重叠时结果可能已保存，
  需要刷新目标目录确认。失败与停止状态保留到用户关闭或重新下载。
...
- 前端测试：导航顺序、目录前进/后退/刷新、失败与并发导航、列表、批量选择、右键菜单、
  查看器分类和保存冲突；远程下载目录快照、已知/未知进度、停止、错误、成功刷新和 URL 不持久化。
