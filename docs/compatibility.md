...
- WordPress、IP+端口反代和其他一键成品站统一使用脚本固定非交互协议。
  静态站、PHP、域名反代、负载均衡和重定向仍存在 KPanel 自编流程，
  已在 `external-config-sources.md` 登记为冻结迁移债务。
...
- 复杂 Nginx 结构若尚无可靠补丁适配器，会明确返回“结构更新适配器未实现”；不得描述为
  “外部资源只读”。宿主机终端已按独立鉴权与集群 scope 实现；交互式容器 TTY 和 Compose
  通用编辑器仍属于待实现能力。
