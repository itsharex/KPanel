...
- `kpanel-browser-relay` 以独立非 root 容器运行，只接受短期签名会话访问公网 HTTP(S)
  目标；它与 Panel 使用不同的浏览器 Origin，共享随机密钥只读挂载且不进入环境变量。
...
`kejilion-panel`、`kejilion-browser-relay` 容器，以及 `kejilion-panel-internal`、`kejilion-panel-egress`
两张网络。安装器还会把固定
...
Panel 与 Relay 容器默认都不发布宿主端口。需要在宿主机或 host-network Nginx
中配置两个不同 HTTPS Origin：Panel 域名反向代理到默认
`http://172.29.255.242:8080`，Relay 域名反向代理到
`http://172.29.255.243:8090`。Panel 代理需设置 `Host`、`X-Real-IP` 和
...
- `kejilion-panel-internal`：固定 `/28` 内部网段，承载宿主机反向代理到 Panel 与
  Relay 的两个独立入口。
...
同时写入内部网络、网关、Panel/Relay 私网地址和可信代理 CIDR，不能在安装后手工改其中一项。
...
KEJILION_PANEL_BROWSER_RELAY_URL=http://154.36.153.9:8081
KEJILION_PANEL_SECURE_COOKIE=false
KEJILION_PANEL_BIND_ADDRESS=0.0.0.0
KEJILION_PANEL_PORT=8080
KEJILION_BROWSER_RELAY_BIND_ADDRESS=0.0.0.0
KEJILION_BROWSER_RELAY_PORT=8081
...
默认情况下，`KEJILION_PANEL_PUBLIC_URL` 与 `KEJILION_PANEL_BROWSER_RELAY_URL`
必须分别与浏览器访问的两个来源完全一致，且二者不能同源。应用市场的
...
docker --host unix:///var/run/docker.sock compose \
  ...
  --env-file /opt/kejilion-panel/.env \
  -f /opt/kejilion-panel/compose.yml \
  exec -T browser-relay /kpanel-browser-relay healthcheck
curl --noproxy '*' --fail --silent http://172.29.255.242:8080/api/v1/health
curl --noproxy '*' --fail --silent http://172.29.255.243:8090/healthz
curl --fail --silent --show-error https://panel.example.com/api/v1/health
curl --fail --silent --show-error https://browser.example.com/healthz
```

还需人工确认：

- 登录、注销、失效 Session 和 CSRF 拒绝；
- 桌面模式新建浏览器窗口，确认导航、前进/后退、刷新、多标签和系统浏览器兜底；
...
标签同时匹配时停止 Panel 与 Relay 容器；随后复核容器运行态、Agent `ActiveState` 和
...
docker --host unix:///var/run/docker.sock logs kejilion-browser-relay
