...
| 3 | DNS 设置不可用 | `kejilion.sh` 协议 + Agent 权限 | 使用 `KJ_DNS_NONINTERACTIVE=1`；升级安装时同步受信脚本；Agent 增加静态 DNS 所需 `CAP_LINUX_IMMUTABLE`；覆盖 resolved 与 static 两种模式 | 已实现，待 Linux 生命周期和线上实机验收 |
| 4 | 应用提示更新脚本、接受许可后仍无法安装 | `kejilion.sh` 协议 + KPanel 升级链路 | KPanel 安装/更新同步固定提交和 SHA-256 的脚本，只从本机可信且已授权脚本继承许可，不静默代替用户授权 | 已实现，待生命周期验收 |
...
| 11 | 体检提示协议不可用 | 主要由新版 `kejilion.sh` 解决 | 使用固定体检目录和 `KJ_TEST_NONINTERACTIVE=1`，升级后同步脚本；禁止自定义 Shell | 新版脚本已提供协议，待 Linux 生命周期验收 |
...
| 14 | KPanel 更新后宿主脚本能力仍旧、缺少回滚 | KPanel 安装/更新链路 | 镜像内同时携带 Agent 与固定摘要脚本；安装、旧版升级、失败回滚、权限和许可继承均纳入生命周期测试 | 已实现，待 CI 容器验收 |
...
| 16 | GitHub CI/Release、Docker Hub 和线上产物未闭环 | 发布链路 | 全量 Go/前端测试、生命周期测试、双架构镜像、OCI 标签、镜像内脚本摘要、Docker Hub `latest`、GitHub Release、线上配置同步逐项验证 | 待发布 |
...
- [ ] `packaging/tests/app-conf-lifecycle.sh`
- [x] Linux amd64/arm64 构建
- [ ] 镜像非 root、健康检查、版本/提交/脚本 OCI 标签
- [ ] 镜像内 `kejilion.sh` SHA-256 与固定值一致
- [ ] GitHub CI 成功
- [ ] GitHub Release 成功
- [ ] Docker Hub 版本标签与 `latest` 摘要一致
- [ ] `kejilion/apps` 中 `kpanel.conf` 与发布版本一致
