...
```text
待分析 -> 已就绪 -> 执行中 -> 待验证 -> 待集成 -> 候选冻结 -> 产物已发布 -> 生产已验证
             |          |          |          |
             +------ 等待输入 / 阻塞 / 废弃 --+
```
...
- “生产已验证”表示已授权目标完成备份、部署、健康、数据/配置和回滚观察；公开镜像 E2E 不能替代。
- `prod-108`（108 服务器）固定只承载正常生产运行，不纳入线上测试、候选验收、灰度、canary、
  兼容性矩阵、失败注入、压力测试、测试账号/数据或持续观察。只有用户逐次明确授权稳定版本生产部署时，
  才执行必要备份、标准更新和最小非侵入式健康/版本/数据完整性/回滚就绪核对；这些核对不是测试证据。
  发布测试和灰度使用隔离真机、154 或另行授权的专用测试环境。
...
3. 对应 L0-L3 和受影响用户旅程通过，命令、环境、精确提交/差异和失败项可复核；
4. 已验证事实、分析结论、建议和未验证风险分开；自动、实机、公开产物、生产证据不混写；
