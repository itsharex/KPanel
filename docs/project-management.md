...
- “生产已部署”表示已授权目标完成备份、标准部署和部署安全核对；它不新增质量验证证据，公开镜像 E2E
  也不能替代部署事实。
- 验证目标必须登记在 `environment-policy.json`。`prod-108`（含别名 `108`）固定只承载正常生产运行，
  不参与候选/功能验收、浏览器、灰度、canary、兼容、失败注入、性能/压力、测试数据或持续观察。
  只有用户逐次授权稳定版本部署时才执行备份、标准更新和最小部署安全核对；发布测试使用 `arena-154`
  或另行登记并授权的专用隔离环境。
