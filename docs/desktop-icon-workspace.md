...
- 状态：已实现，待 L3 上线审批
- 适用范围：宽屏桌面图标布局、已安装应用与网站入口显隐、站点别名、URL 及文件快捷方式
...
文件和目录入口只是指向 Agent 文件管理真实路径的快捷方式，不复制、不移动、不上传目标，也不建立
宿主机真实 `Desktop` 目录。本期不提供用于收纳图标的虚拟桌面文件夹、多选框选、像素级自由重叠、
多人独立桌面、跨主机同步或远程图标抓取；不改变经典模式、应用安装状态、网站配置、Nginx、Docker
...
- 文件路径前后端都只接受规范绝对 POSIX 路径，继续复用文件管理既有根目录约束、保护路径、符号链接
  ...
  既有预览规则读取，目录只列出当前目录。路径不会被拼入 Shell，也不会写审计。
...
桌面空白区。拖放采用同页面内存令牌，只信任当前文件管理产生的数据；拖到窗口、组件或任务栏不
创建入口。添加是单次 workspace PUT，重复目标不重复创建，超过 64 个快捷方式时整批失败且不部分
...
| 文件与目录快捷方式 | 文件管理单项/多选/当前目录添加；拖到桌面空白区创建且不移动真实目标；同一路径去重、整批限额失败；文件打开父目录并预览、目录直接打开；不同目标独立窗口、相同目标聚焦复用；删除/改名/权限/类型变化明确失败；从桌面移除不改变文件系统 |
| 路径与拖放安全 | 仅规范绝对 POSIX 路径；拒绝控制字符、反斜杠、空/点/父级组件、超长路径及 URL/路径混用；保护路径和符号链接沿用文件管理规则；伪造外部拖放 MIME/令牌不创建入口；Panel/Agent 不执行 Shell、不跟随路径创建副本 |
