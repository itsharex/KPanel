...
管理员可以在宽屏桌面拖动图标、交换位置、自动排列或重置布局；刷新、重新登录和 Panel 重启后
...
参与自动整理；自动整理应停止并提示限制，不发 PUT、不报告成功。重置布局提交空 `positions`，随后
使用同一规则从来源顺序派生默认位置。滚动偏移不属于 workspace，不得误写为图标位置。
...
- `window.innerWidth <= 760`：始终按入口顺序逐页派生紧凑滚动布局，不读取为独立位置档位，也不把
  派生结果回写。拖拽、键盘移动、自动排列和重置位置在此宽度禁用；返回宽屏后恢复原宽屏位置。
...
触摸与手写笔保留单击打开和长按菜单；宽屏下必须先进入“整理图标”模式才允许拖动，阈值为 12 px。
...
请求，只在放下、自动排列、重置、显隐或编辑确认后提交。最大数据集下需验证 256 KiB metadata、
...
做最终校验。功能保持独立 schema 和目录，避免旧版本整体写回其他状态文件时丢失新字段。当前仅进入
开发分支和本地预览，不推送 `main`、不打标签、不发布镜像、不部署生产环境。
...
| 鼠标与键盘 | 真实 Chromium/Firefox 验证阈值前不 capture，普通 click/dblclick 仍命中子按钮；6 px 后才 capture 并拖拽，放下不误打开；同页/跨页交换、边缘自动滚动、边界夹取、自动排列、重置、Ctrl/Command+方向键、Enter/Space、菜单键和焦点/播报正确 |
| 触摸与手写笔 | 单击打开、长按菜单、宽屏整理模式、12 px 阈值、多指、`pointercancel`、失焦与滚动边界不误开或误保存；窄屏不允许重排 |
