...
上传到当前目录；文件管理内部项目拖到桌面是“仅创建入口”；应用、网站、URL 入口或桌面文件入口拖入
文件窗口不生成 `.desktop`/`.url`、不复制、不移动真实目标。上传传输可取消当前 XHR；Agent 仍以同目录
...
| 路径与拖放安全 | 仅规范绝对 POSIX 路径；拒绝控制字符、反斜杠、空/点/父级组件、内部临时名前缀和超长路径；保护路径、只读目录和符号链接沿用文件管理规则；内部令牌与外部 `Files` 数据不混用，本地绝对路径不上传；Panel/Agent 不执行 Shell，取消和失败不留下临时文件 |
