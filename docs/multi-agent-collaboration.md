...
智能体之间不需要读取彼此的私有对话；它们通过 Issue、Git 和 CI 交换可验证状态。
...
1. 协调中心从 `.github/ISSUE_TEMPLATE/ai-task.yml` 创建 AI Task Issue。
2. Issue 明确目标、路径声明、基线、风险等级、当前智能体、权限、依赖和验收。
3. 协调中心创建专用 branch/worktree，并把绝对路径写回 Issue。
4. Codex 或 Claude 只在自己的 worktree 写入，阶段结果以提交保存。
5. 智能体把提交哈希、测试和风险写回 Issue，将状态改为待复核。
6. 另一智能体或独立验证任务从提交重建环境并复核，不接管原会话的未提交状态。
7. 协调中心批准后进入集成队列；发布任务只接收精确提交清单。
...
- 实际需要越出声明范围时，先更新 Issue 并由协调中心重新检查冲突，不得直接扩大范围。
...
当前状态：state:review / state:blocked
基线：<commit>
分支：<branch>
最新提交：<commit>
...
- GitHub Issue 不可访问：暂停新并行写任务。
- 负责人失联：协调中心把 Issue 标记为 blocked，确认分支和工作树后再移交。
- 两个智能体修改同一文件：停止其中一方，分别保存聚焦提交，由集成任务决定顺序和冲突处理。
- 智能体声称完成但无提交或证据：状态保持开发中，不进入集成。
- 发布期间出现新功能：进入下一版本 Issue，不修改冻结候选。
